...
function Base.getindex(buffer::StringBuffer, i::Integer)
    !_in_bounds(buffer, i) && throw(BoundsError("Bound error size: $size idx: $i"))
    Char(buffer.value[i])
end
...
function Base.append!(buffer::StringBuffer, str::String)
    ...
    unsafe_copyto!(buffer.value, buffer.size + 1, unsafe_wrap(Vector{UInt8}, str), 1, ncodeunits(str))
    buffer.size = required_size
end
...
_in_bounds(buffer::StringBuffer, i::UnitRange) = first(i) >= 1 && last(i) <= buffer.size
